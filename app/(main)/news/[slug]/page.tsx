...
const newsDetails: Record<string, {
  ...
  "2026-02-14-sano-pancreatic": {
    ...
    comment: "局所進行膵癌に対する治療には,化学放射線療法と全身化学療法がありますが,いずれがより有効であるかについては,現時点で明確な結論は得られていません.\n\n本研究では,局所進行膵癌を対象とした2つのランダム化���������II相試験である,JCOG1106「局所進行膵癌に対するS-1併用放射線療法における導入化学療法の意義に関するランダム化第II相試験」と,JCOG1407「局所進行膵癌を対象としたmodified FOLFIRINOX療法とゲムシタビン＋ナブパクリタキセル併用療法のランダム化第II相試験」の統合解析を行い,S-1併用���射���療�����と���ム���タビン＋ナブパクリタキセル併用（GnP）療法の有効性・安全性を比較検討しました.\n\nその結果,GnP療法は微小転移の抑制において優位性を示す可能性がある一方で,S-1併用放射線療法は局所病変の制御に有益である可能性が示唆されました.\n\n最後に,本研究の立案から論文化に至るまでご指導いただきました,神奈川県立がんセンターの小林先生,上野先生,ならびに前田教授に心より感謝申し上げます。（佐野 裕亮）"
  },
"2026-02-08-shibui-uc-biomarker": {
  ...
  "2026-01-23-endo-hepatic-cyst": {
    ...
    abstract: "背景と目的：巨大肝嚢胞による胆管閉塞および胆管炎に対し、外科的手術や経皮的ドレナージが検討されますが、嚢胞が血管や胆管を強く圧排している場合、経皮的穿刺は誤穿刺のリスクを伴います。本報告では、そのような困難症例に対し、超音波内視鏡（EUS）ガイド下での段階的なドレナージを行い、良好な経過を得た症例を提示しました。\n\n��法：肝嚢胞感染を合併した巨大肝嚢胞に対し、初回治療としてEUSガイド下肝嚢胞ドレナージを選択。嚢胞の縮小に合わせて段階的に処置を進めるステップバイステップのアプローチを施行しました。\n\n結果・結論：EUSを用いることで、介在する血管や胆管を回避しながら安全に嚢胞内に到達し、効果的なドレナージを行うことができました。巨大感染性肝嚢胞に対するEUS治療の報告はまだ限られていますが、本症例��ように���囲臓器への圧排が強い症例においては、低侵襲かつ安全な代替療法となり得ることが示唆されました。",
  ...
  "2026-01-09-kobayashi-guidewire": {
    ...
    abstract: "背景と目的：十二指腸の浅表性非乳頭部上皮性腫瘍（SNADET）に対する内視鏡的粘膜下層剥離���（ESD）は、特に主乳頭近傍の病変において技術的���困難であり、偶発症のリスクが高い。本研究の目的は、主乳頭近傍のSNADETに対する、ESD施行前の予防的膵管ステント留置を組み合わせた治療戦略の有効性と安全性を評価すること���す。\n\n方法：2014年3月か��2023年9月の間に、主乳頭近傍（10mm以内）のSNADETに対し、予防的膵管ステント留置後にESDを施行した11例を対象に後方視的解析を行いました。\n\n結果・結論：全例（100%）で一括切除に成功し、R0切除率は90.9%であった。ステント留置からESDまでの期間の中央値は2日であった。ESD中のステント脱落は認められず、全例でクリップを用いた完全な縫縮が可能であった。偶発症として、遅発性出血が1例、術後膵炎が2例に認められた。本手法は、主乳頭近傍のSNADETに対する有効な治療戦略となり得るが、術後膵炎のリスクは依然として存在し、さらなる予防策の検討が必要である��とが示唆されました。",
    comment: "十二指腸ESDは,皆様もご存じのとおり偶発症のリスクが高く,技術的難易度も高い内視鏡治療の一つです.なかでも,十二指腸主乳頭近傍病変に対するESDでは,乳頭浮腫による術後膵炎や,ESD潰瘍創部の縫縮時に乳頭を巻き込むリスクが問題となります.\n\nそこで,これらの偶��症予防を目的として,我々は十二指腸主乳頭近傍病変に対するESDに際し,まずERCPにより膵管ステントを留置し,数日後にESDを施行する治療戦略をとりました.今回は,本手法を用いた11例の治療成績について報告しております.\n\nERCPによる膵管ステント留置および十二指腸ESDはいずれも高い専門的技術を要する手技であり,本治療は各専門グループ間の緊密な連携があって初めて成立するものと思います.\n\n診療にあたっていただいた肝胆膵グループの先生方,管グループの先生方,また論文執筆をサポートいただいた三輪先生,平澤先生,前田教授に感謝申し上げます.（小林亮介）"
  },
